  ...
  return (
    <>
      <Form
        ...
        style={{ margin: "10px" }}
      >
          <Form.Item name="contractId" label="Müqavilə" rules={[{ required: true }]}>
            <Select
                showSearch
                loading={loading}
                ...
                style={{ width: 200 }}
                onChange={(value, option) => form.setFieldsValue({ ContractId: value })} // update the form field value
                value={form.getFieldValue("contractId")} // get the field value from the form
            />
          ...
          <Form.Item name="paymentAmount" label="Ödəniş məbləği" rules={[{ required: true }]}>
            <Input type="number"  placeholder="Ödəniş məbləöi daxil edin"/>
          </Form.Item>
          ...
          <Form.Item name="description" label="Açıqlama" rules={[{ required: true }]}>
            <Input placeholder="Açıqlama qeyd edin"/>
          </Form.Item>
